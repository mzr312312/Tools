...
try:
    ...
    from PIL import Image, ImageTk  # 导入 Pillow (PIL) <-- 【已修改】

    PYGAME_LOADED = True
    PLYER_LOADED = True
    PYSTRAY_LOADED = True  # 新增 pystray 加载状态
except ImportError:
    print("错误：缺少必要的库。请运行 'pip install ttkbootstrap pygame plyer pystray pillow'")
    PYGAME_LOADED = False
    PLYER_LOADED = False
    PYSTRAY_LOADED = False  # 新增 pystray 加载状态
            ...
            if not self._is_paused:
                ...
                if self.interval_time_left <= 0:
                    if self.current_phase == "WORK":
                        ...
                        self.app.send_notification("休息结束", "新一轮工作开始！")
...
class FocusApp(tb.Window):
    ...
    def stop_session(self, completed=False):
        ...
        if completed:
            ...
            self.send_notification("会话完成！", "恭喜你完成了一次专注会话！")  # 错别字已修正
        else:
    ...
    def hide_window(self):
        ...
        self.withdraw()
        self.send_notification("专注助手", "已最小化到系统托盘。")
